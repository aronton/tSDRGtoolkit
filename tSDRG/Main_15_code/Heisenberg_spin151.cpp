...
void Jlist(const paralist& para, datalist& data)
{
    random_device rd;             // non-deterministic generator.
    long long seed = rd();
    data.seed = para.sample;
    mt19937 genRandom(data.seed);    // use mersenne twister and seed is rd.
...
void tSDRG_XXZ(const paralist& para, datalist& data)
{
    ...
    tSDRG(MPO_chain, data.energy, w_up, w_loc, chi, to_string(J), Pdis, sample, save_RG_info, info);
